...
def add_features(df):
    ...
    df['chi_vs_length'] = df.apply(lambda row: float(row['chinese'])/float(row['total_length']),
                                axis=1)
    df['num_words'] = df.内容.str.count('\S+')
    df['num_unique_words'] = df['内容'].apply(lambda comment: len(set(w for w in comment.split())))
